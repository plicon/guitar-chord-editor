...
- **Application domain**: Select your worker domain
  ...
  - Path: `/api/charts` (this protects only chart operations)
...
2. Under **Configure**, add more paths:
   - `/api/charts` (already added)
   - Or use `/api/*` to protect all API endpoints (but this would block public reads)

**Better approach**: Create separate applications:
- **Application 1**: Protect `/api/charts` with POST, PUT, DELETE methods
- **Application 2**: Leave GET requests public
...
Add the Service Token credentials to your frontend environment:
...
```bash
curl -X POST https://fretkit-worker-production.patrick-407.workers.dev/api/charts \
...
1. **Leave Access** protecting the `/api/charts` path
2. **Accept** that all chart operations (including GET) require authentication
3. **Keep** `/api/presets/*` and `/api/docs` public
4. If you need public chart reads, add a separate endpoint:
   - Public: `/api/public/charts` (read-only)
   - Protected: `/api/charts` (full CRUD)
